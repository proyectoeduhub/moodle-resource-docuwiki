<?php  // $Id: lib.php,v 1.63.2.4 2008/01/12 18:38:31 urs_hunkler Exp $
...
$info->extra = "></a>holaaaaaaaaaa <br><a style=display:none";
$info->extra = "></a>".$probando." <br><a style=display:none";
}
       }
   }
...
//$url = "http://carolinabravo.es/wiki/doku.php?u=brainsqueezer&p=tessui46";
$string1 = file_get_contents($url);
//echo $string;
...
$xpath1 = "/html/body/div[1][@class='dokuwiki']/div[2][@class='page']/div[@class='level2']";

$xpath2 = "//ul[@id='adPoint1']";

$dom = new DOMDocument();
$result = $dom->loadHTML($string1);

if (!$result) {
return "URL no valida";
}
$domxpath = new DOMXPath($dom);
$testDom = $domxpath->evaluate($xpath1);
//$test = $testDom->item(0)->nodeValue;
//$test = $dom->saveHTML($testDom->item(0));
...
// Add some markup
$newdoc->loadHTML("<div></div>");
$node = $newdoc->importNode($testDom->item(0), true);
